...
\hline
  ...
  {\tt cost} &  Array of cost blocks, where & Y & T &  \\
    ...
    &  1) marginal  cost (\$/p.u.-hr), and 2) block size (Array of Array of Float Float) &  &   &$c^{en}, p^{max}$ \\
...
\begin{tabular}{ l | l | c | c | c |}
  ...
  \hline
  {\tt p\_reg\_res\_up\_cost} & Costs for regulation reserve up (Float, \$/pu-h) & Y & S & $c^{rgu}$\\
  {\tt p\_reg\_res\_down\_cost} & Costs for regulation reserve down (Float, \$/pu-h) & Y & S & $c^{rgd}$\\
  {\tt p\_syn\_res\_cost}        & Costs for synchronized reserve (Float, \$/pu-h) & Y & S &{ $c^{scr}$}\\
  {\tt p\_nsyn\_res\_cost}       & Costs for non-synchronized reserve (Float, \$/pu-h) & Y & S &{ $c^{nsc}$}\\
  {\tt p\_ramp\_res\_up\_online\_cost}    & Costs for ramp up reserve when online (Float, \$/pu-h) & Y & S & $c^{rru, on}$\\
...
\begin{tabular}{ l | l | c | c | c |}
  ...
  {\tt select\_ub} & Mode selection upper bound (Array of Binary) & Y & T & $u^{md,max}$\\
  {\tt select\_lb} & Mode selection lower bound (Array of Binary) & Y & T & $u^{md,min}$\\
  {\tt mode\_transition\_uids} & Array of feasible mode uids to transit to (Array of String) & Y & S & $u^{tr,min}, u^{tr,max}$\\
  ...
  {\tt cost} &  Array of cost blocks, where & Y & T &  \\
    ...
    &  a) marginal cost (\$/p.u.-hr), and b) block size (p.u.) (Array of Array of Float Float)&  &   &$c^{en}, p^{max}$ \\
...
%  &  FLEXI-RAMP-UP, FLEXI-RAMP-DOWN ) &  & & \\
  {\tt REG\_UP} & Regulation reserve up requirement (Fraction, Float) & Y & T & $\sigma^{rgu}$ \\
  {\tt REG\_DOWN} & Regulation reserve down requirement (Fraction, Float) & Y & T & $\sigma^{rgd}$ \\
  {\tt SYN} & Synchronized reserve requirement (Fraction, Float) & Y & T & { $\sigma^{scr}$} \\
  {\tt NSYN} & Non-synchronized reserve requirement (Fraction, Float) & Y & T & { $\sigma^{nsc}$} \\
  {\tt RAMPING\_RESERVE\_UP} & Ramping reserve up requirement (Float) & Y & T & $p^{rru,min}$ \\
  {\tt RAMPING\_RESERVE\_DOWN} & Ramping reserve down requirement (Float) & Y & T & $p^{rrd,min}$ \\
  \hline
...
used by the output data format are listed below.
\begin{todo}[]{}
Question: Will competitors need to declare the total amount of reserve per region/per time period?
...
\small
\begin{tabular}{ l | l | c | c | c |}
Active reserve output attributes & Description (Type) & Req & Sec & Sym\\
\hline
  {\tt uid} & Region reserve unique identifier (String) & Y & B & \\
  {\tt REG\_UP} & Regulation reserve up procurement (Fraction, Float) & Y & T & $\sigma^{rgu}$ \\
  {\tt REG\_DOWN} & Regulation reserve down procurement (Fraction, Float) & Y & T & $\sigma^{rgd}$ \\
  {\tt SYN} & Synchronized reserve procurement(Fraction, Float) & Y & T & { $\sigma^{scr}$} \\
  {\tt NSYN} & Non-synchronized reserve procurement(Fraction, Float) & Y & T & { $\sigma^{nsc}$} \\
  {\tt RAMPING\_RESERVE\_UP} & Ramping reserve up procurement (Float) & Y & T & $p^{rru,min}$ \\
  {\tt RAMPING\_RESERVE\_DOWN} & Ramping reserve down procurement (Float) & Y & T & $p^{rrd,min}$ \\
  \hline
\end{tabular}
\end{center}
