...
\hline
  ...
  {\tt interval\_duration} & Time duration of the intervals, per time period in hours & Y & T & $d_t$ \\
                           & (Array of Float) &  &  & \\
                           \hline
                           {\tt timestamp\_start} & Period beginning timestamp for the first interval with Timestamp format YYYY-MM-DDThh:mm at UTC (Timestamp) & N & S & \\
                           {\tt timestamp\_stop} & Period beginning timestamp for the interval following the last interval with Timestamp format YYYY-MM-DDThh:mm at UTC (Timestamp) & N & S & \\
\hline
...
%  &  FLEXI-RAMP-UP, FLEXI-RAMP-DOWN ) &  & & \\
  {\tt REG\_UP} & Regulation reserve up requirement (Float) & Y & S & $\sigma^{rgu}$ \\
  {\tt REG\_DOWN} & Regulation reserve down requirement (Float) & Y & S & $\sigma^{rgd}$ \\
  {\tt SYN} & Synchronized reserve requirement (Float) & Y & S & { $\sigma^{scr}$} \\
  {\tt NSYN} & Non-synchronized reserve requirement (Float) & Y & S & { $\sigma^{nsc}$} \\
