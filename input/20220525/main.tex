...
\hline
  ...
  {\tt interval\_duration} & Time duration of the intervals, per time period in hours & Y & T & $d_t$ \\
                           & (Array of Float) &  &  & \\
                           \hline
  {\tt timestamp\_start} & Period beginning timestamp for the first interval {\color{red}(String: Timestamp, YYYY-MM-DDThh:mm at UTC)} & N & S & \\
  {\tt timestamp\_stop} & Period beginning timestamp for the interval following the last interval {\color{red}(String: Timestamp, YYYY-MM-DDThh:mm at UTC)} & N & S & \\
\hline
...
\hline
{\tt base\_norm\_mva} & Base MVA normalization constant (Float)  & {\color{red} Y} & S &  \\
  \hline
\end{tabular}
\end{center}
