...
\hline  
  {\tt p\_vio\_cost} & System violation costs for active power violation (\$/p.u.) & Y & S &$c^{p,+}$ \\
    ...
    %&  a) violation cost (\$/p.u.-hr), and b) block size (p.u.) &  &   &$c^{p,+}$, $p^{+,max}$ \\
  {\tt q\_vio\_cost} & System violation costs for reactive power violation (\$/p.u.) & Y & S & $c^{q,+}$\\
%      &  each cost block is an array with exactly two elements:   &  &   & \\
%    &  a) violation cost (\$/p.u.-hr), and b) block size (p.u.) &  &   &$c^{q,+}$, $q^{+,max}$ \\    
  {\tt p\_bus\_vio\_cost} & Bus violation costs for active power violation (\$/p.u.)& Y & S & $c^{p,+}$\\
%      &  on buses, each cost block is an array with exactly two elements:   &  &   & \\
%    &  a) violation cost (\$/p.u.-hr), and b) block size (p.u.) &  &   &$c^{p,+}$, $p^{+,max}$ \\
...
%      &  each cost block is an array with exactly two elements:   &  &   & \\
%    &  a) violation cost (\$/p.u.-hr), and b) block size (p.u.) &  &   &$c^{v,+}$, $v^{+,max}$ \\    
  {\tt mva\_branch\_vio\_cost} & Branch violation costs for thermal violation (\$/p.u.)  & Y & S &$c^{s,+}$  \\
%      &  each cost block is an array with exactly two elements:   &  &   &  \\
%      &  a) violation cost (\$/p.u.-hr), and b) block size (p.u.) &  &   &$c^{s,+}$, $s^{+,max}$ \\    
\hline
...
% \begin{todo}[]{}
% The output attribute list below is a preliminary conversion from the formulation model variables. 
% Not all of them are required, and most/all of them hence are marked with a 'N' in the Req column.
% Those that are needed to be reported by competitors will be marked with a 'Y' (in the future).
% The intention of listing all the attributes below is to allow competitors to report any debatable solution values
% to the organization committee for debugging/validation purposes.
% \end{todo}
% \begin{center}
% \small
% \begin{tabular}{ l | l | c | c |}
% Global output attributes & Description (Type) & Req & Sym\\
% \hline
%   {\tt obj\_cost}   & Total minimization objective (Float, USD) & N & $z$ \\
%   {\tt p\_vio}      & System active power violation (Array of Float, pu) & N & $p$ \\
%   {\tt p\_vio\_pos} & System active power positive violation (Array of Float, pu) & N & $p^{+}$ \\
%   {\tt p\_vio\_pos\_costblock} & System active power positive violation per cost block & N & $p^{+}$ \\
%                               & (Array of Float per cost block, pu) & &  \\
%   {\tt p\_vio\_neg} & System active power negative violation (Array of Float, pu) & N & $p^{-}$ \\
%   {\tt p\_vio\_neg\_costblock} & System active power negative violation per cost block & N & $p^{-}$ \\
%                               & (Array of Float per cost block, pu) & &  \\                
%   {\tt q\_vio}      & System reactive power violation (Array of Float, pu) & N & $q$ \\
%   {\tt q\_vio\_pos} & System reactive power positive violation (Array of Float, pu) & N & $q^{+}$ \\
%   {\tt q\_vio\_pos\_costblock} & System reactive power positive violation per cost block & N & $q^{+}$ \\
%                               & (Array of Float per cost block, pu) & &  \\
%   {\tt q\_vio\_neg} & System active power negative violation (Array of Float, pu) & N & $p^{-}$ \\
%   {\tt q\_vio\_neg\_costblock} & System reactive power negative violation per cost block & N & $q^{-}$ \\
%                               & (Array of Float per cost block, pu) & &  \\                                               
...
\begin{todo}[]{}
(Archived) Discussion on participation/loss factor is still open. Proposals: a) each bus has one, b) each dispatchable unit has one, or c) fixed globally in the data formulation/data format.
\end{todo}
\begin{center}
\small
\begin{tabular}{ l | l | c | c | c |}
...
%  {\tt va} & Voltage angle (Float, radian)& Y & T & \\ 
  ...
  {\color{red} {\tt con\_loss\_factor}} & Contingency participation loss factor (Float) & Y & S & $\alpha$\\
  {\tt area} &  Bus control area (String)& Y & S & \\
  {\tt zone} &  Bus control zone (String)& Y & S & \\ 
  {\tt reserve\_uid} & Bus regional reserve unique identifier  & Y & S & \\ 
                     &  (Array of String) &  & & \\ 
  \hline
  Location/Operation information & Description (Type) & Req & Sec & Sym\\
  \hline
  {\tt\color{red} base\_nom\_volt} & Bus nominal voltage (Float) & N & S & \\
...
\hline
  ...
  {\tt bus} & Unique identifier for connecting bus (String)& Y & S &  \\            
  {\tt gs}  & Shunt conductance (p.u.) & Y & S & $g^{sh}$ \\
  {\tt bs}  & Shunt susceptance (p.u.) & Y & S & $b^{sh}$ \\
...
\subsection{Dispatchable Devices: Generating Units, Load Demands, and Storage Devices}
\label{nom:gen}
\paragraph{Input Attributes} The attributes for dispatchable devices used by the input data format are listed below.
\begin{todo}[]{}
A preliminary version with state/configuration is drafted. 
Will need to modify based on the latest formulation document version.\\
\end{todo}
\begin{todo}[]{}
{\color{red} To be decided: One general dispatchable device JSON object vs consumer and prosumer JSON objects}\\
\end{todo}
\begin{todo}[]{}
{\color{red} To be decided: Explicit extra JSON structure in the static file section to store data for the first time point vs Implicit time series including the first time point in the time series file section.}
\end{todo}
...
%   {\tt startup\_cost\_hot} & Cost for hot start (Float, \$)   & N & T & $c^{sus}$ \\
%   {\tt startup\_cost\_warm} & Cost for warm start (Float, \$) & N & T & $c^{sus}$\\      
%   {\tt startup\_cost\_cold} & Cost for cold start (Float, \$) & Y & T & $c^{sus}$\\
...
%  {\tt startup\_time\_hot} & Time (hr) to hot start (Float) & N & S & \\
%  {\tt startup\_time\_warm} & Time (hr) to warm start (Float) & N & S & \\
%  {\tt startup\_time\_cold } & Time (hr) to cold start (Float) & N & S & \\
  {\tt in\_service\_time\_ub} & Maximum time (hr) to operate/in service (Float)& Y & S &  $d^{up,max}$ \\ 
  {\tt in\_service\_time\_lb} & Minimum time (hr) to operate/in service (Float) & Y & S & $d^{up,min}$\\
...
\begin{tabular}{ l | l | c | c | c |}
 \hline
   Optional config. attributes & Exist if {\tt q\_linear\_cap} is true &  &  & \\
  \hline
  {\tt\color{red} q\_0}               & Reactive dispatch at zero active dispatch (Float, p.u.) & Y & S & $q^{0}$\\
  {\tt\color{red} pq\_ratio}          & Sensitivity of reactive w.r.t active dispatch (Float) & Y & S & $\beta$\\
  \hline \hline
   Optional config. attributes & Exist if {\tt q\_bound\_cap} is true &  &  & \\
  \hline
  {\tt\color{red} q\_0\_ub}           & Maximum reactive dispatch at zero active dispatch (Float, p.u.) & Y & S & $q^{max,0}$\\
  {\tt\color{red} q\_0\_lb}           & Minimum reactive dispatch at zero active dispatch (Float, p.u.) & Y & S & $q^{min,0}$\\
  ...
  \hline 
  \hline
   Optional config. attributes & Exist if {\tt is\_storage} is true &  &  & \\
  \hline
  {\tt energy\_ub} & Upper bound of energy storage (Float, p.u.)& Y & T & $e^{max}$\\
  {\tt energy\_lb} & Lower bound of energy storage (Float, p.u.)& Y & T & $e^{min}$\\ 
...
% \label{nom:storage}
% \paragraph{Input Attributes}
% The attributes for storage facilities used by the 
...
% Input attributes & Description (Type) & Req & Sec & Sym\\
% \hline
%   {\tt uid} & Storage unique identifier (String) & Y & B &  \\
%   {\tt bus} & Unique identifier for connecting bus (String) & Y & S & \\
% %  {\tt ps} & Active power injection (Float, p.u.) & Y & T & \\
% %  {\tt qs} & Reactive power injection (Float, p.u.) & Y & T & \\    
% %  {\tt energy} & Current state of charge (Float, p.u.-hour) & Y & T & \\
% %  {\tt charge} & Current charge (Float, p.u.) & Y & T &  \\
% %  {\tt discharge} & Current discharge (Float, p.u.) & Y & T & \\
% %  {\tt charge\_efficiency} & Charging efficiency rating (Float) & Y & S & \\
% %  {\tt discharge\_efficiency} & Discharging efficiency rating (Float) & Y & S & \\
% %  {\tt energy\_ub} & Maximum state of charge (Float, p.u.-hour) & Y & S & \\
% %  {\tt charge\_ub} & Maximum charge rating (Float, p.u.) & Y & S & \\
% %  {\tt discharge\_ub} & Maximum discharge rating (Float, p.u.) & Y & S & \\
%   {\tt ps\_ub} & Maximum active power injection (Float, p.u.) & Y & T & $p^{max}$\\
%   {\tt ps\_lb} & Minimum active power injection (Float, p.u.) & Y & T & $p^{min}$\\
%   {\tt qs\_ub} & Maximum reactive power injection (Float, p.u.) & N & S & $q^{max}$\\      
%   {\tt qs\_lb} & Minimum reactive power injection (Float, p.u.) & N & S & $q^{min}$\\
%   {\tt q\_0}               & Reactive injection at zero active injection (Float, p.u.) & Y & T & $q^{0}$\\
%   {\tt q\_0\_ub}           & Maximum reactive injection at zero active injection (Float, p.u.) & Y & T & $q^{max,0}$\\
%   {\tt q\_0\_lb}           & Minimum reactive injection at zero active injection (Float, p.u.) & Y & T & $q^{min,0}$\\
%   {\tt ps\_ratio}          & {\color{red} Sensitivity of reactive w.r.t active injection (Float)} & Y & T & $\beta$\\
%   {\tt ps\_ub}             & {\color{red} Upper bound for sensitivity of reactive w.r.t active injection (Float)} & Y & T & $\beta^{max}$\\
%   {\tt ps\_lb}             & {\color{red} Lower bound for sensitivity of reactive w.r.t active injection (Float)} & Y & T & $\beta^{min}$\\
  
%   {\tt ps\_nom\_ramp\_ub}   & Maximum power increase from operating cond.(Float, p.u./hr) & Y & S & $p^{ru,max,ro}$ \\
%   {\tt ps\_start\_ramp\_ub} & Maximum power increase from startup cond. (Float, p.u./hr) & Y & S & $p^{ru,max,su}$\\
%   {\tt ps\_down\_ramp\_ub}  & {\color{red} Maximum power increase from shutdown cond. (Float, p.u./hr)} & Y & S & $p^{ru,max,sd}$\\
%   {\tt ps\_nom\_ramp\_lb}   & Maximum power decrease from operating cond.(Float, p.u./hr) & Y & S & $p^{rd,max,ro}$ \\
%   {\tt ps\_start\_ramp\_lb} & {\color{red} Maximum power decrease from startup cond. (Float, p.u./hr)} & Y & S & $p^{rd,max,su}$\\
%   {\tt ps\_down\_ramp\_lb}  &  Maximum power decrease from shutdown cond. (Float, p.u./hr) & Y & S & $p^{rd,max,sd}$\\  
%   {\tt cost\_st\_model} & Storage cost model type & Y & T & \\
%   &(String: Linear, Polynomial) & & &  \\
%   {\tt cost\_st\_parameters} & Storage cost model parameters & Y & T & $p^{ec,0}$,\\
%                              & (Array of Float)& Y & T & $p^{ec,max}$,$c^{ec}$\\
%   {\tt standby\_cost} & {\color{red} Storage fixed standby cost (Float)} & Y & T & $c^{on}$\\
%   {\tt connection\_cost} & {\color{red} Storage connection/startup cost (Float)} & Y & T & $c^{su}$\\           {\tt disconnection\_cost} & {\color{red} Storage connection/shutdown cost (Float)} & Y & T & $c^{sd}$\\     {\tt in\_service\_time\_ub} & Maximum time (hr) to operate/in service (Float)& Y & S &  $d^{up,max}$ \\ 
%   {\tt in\_service\_time\_lb} & Minimum time (hr) to operate/in service (Float) & Y & S & $d^{up,min}$\\
%   {\tt down\_time\_ub} & Maximum (hr) disconnection/down time (Float) & Y & S & $d^{dn,max}$ \\
%   {\tt down\_time\_lb} & Minimum (hr) disconnection/down time (Float) & Y & S & $d^{dn,min}$\\      
%   {\tt ps\_regulation\_ub} & {\color{red} Maximum regulation reserve (Float, p.u.) }& Y & S & $p^{rgr,max}$\\
%   {\tt ps\_spin\_ub}       & {\color{red} Maximum spinning reserve (Float, p.u.)} & Y & S & $p^{spr,max}$\\ 
%   {\tt ps\_nonspin\_ub}    & {\color{red} Maximum non-spinning reserve (Float, p.u.)} & Y & S & $p^{nspr,max}$\\
%   \hline
% \end{tabular}
% \end{center}
...
% \paragraph{Output Attributes}
% The attributes for storage facilities used by the 
% output data format are listed below.
...
% Output attributes & Description (Type) & Req & Sym\\
% \hline
%   {\tt uid} & Storage unique identifier (String) & Y &  \\
%   {\tt ps} & Active power injection (Float, p.u.)   & Y &  $p$\\
%   {\tt qs} & Reactive power injection (Float, p.u.) & Y &  $q$\\      
%   {\tt ps\_regulation} & {\color{red} Regulation reserve (Float, p.u.) }& Y &  $p^{rgr}$\\
%   {\tt ps\_spin}       & {\color{red} Spinning reserve (Float, p.u.)} & Y & $p^{spr}$\\ 
%   {\tt ps\_nonspin}    & {\color{red} Non-spinning reserve (Float, p.u.)} & Y & $p^{nspr}$\\
%   {\tt status}     & {\color{red} Connection status (String: on, off)} & Y &  $u^{on}$ \\ 
%   {\tt startup}    & {\color{red} Flag to indicate grid connection from disconnection (String: True/False)} & N & $u^{su}$\\
%   {\tt shutdown}   & {\color{red} Flag to indicate grid disconnection from connection (String: True/False)} & N & $u^{sd}$\\ {\tt remain\_on} & {\color{red} Flag to the storage will remain connected (String: True/False)} & N & $u^{ro}$\\  
%   \hline
% \end{tabular}
% \end{center}
...
\hline
  ...
  {\tt res\_vio\_cost} & Reserve violation cost/penalty (Float, \$/p.u.) & Y & S & \\ 
      &$\hookrightarrow$ Case {\tt type}: SPIN &  & &$c^{spru,-} $\\ %, p^{spru,-,max}$ \\ 
      &$\hookrightarrow$ Case {\tt type}: RAMP-UP &  & &$c^{rru,-} $\\%, p^{rru,-,max}$ \\ 
      &$\hookrightarrow$ Case {\tt type}: RAMP-DOWN &  & &$c^{rrd,-} $\\%, p^{rrd,-,max}$ \\ 
      &$\hookrightarrow$ Case {\tt type}: REG-UP &  & &$c^{rgru,-} $\\%, p^{rgru,-,max}$ \\ 
      &$\hookrightarrow$ Case {\tt type}: REG-DOWN &  & &$c^{rgrd,-} $\\%, p^{rgrd,-,max}$ \\ 
      &$\hookrightarrow$ Case {\tt type}: CONT &  & &$c^{cgru,-} $\\%, p^{cgru,-,max}$ \\ 
      &$\hookrightarrow$ Case {\tt type}: REACT-UP &  & &$c^{qru,-} $\\%, q^{qru,-,max}$ \\ 
      &$\hookrightarrow$ Case {\tt type}: REACT-DOWN &  & &$c^{qrd,-}$\\%, q^{qrd,-,max}$ \\      
      \hline
\end{tabular}
\end{center}    
...
% \section{JSON Format}
% This document presents a draft of the data format for the ARPA-E GO Competition
% Challenge 3. This document is not final and expects to be updated continuously. 

% The competition employs JSON for storing and representing various data 
% for the competition. 
% In JSON, data values must be one of the following data types:
% strings, numbers, JSON objects, arrays, booleans, or null.
% The example below showcases a JSON object file with 6 keys (key1 to key6),
% with each corresponds to a different data types. 
...
% }
% \end{verbatim}
% Details of the JSON standard and parsing rules can be found in \url{https://www.json.org/json-en.html}.
% Section~\ref{sec:format_spec} will showcase the latest proposal for the data format.
