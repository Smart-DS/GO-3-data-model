...
\hline
  ...
  {\tt pg\_ramp\_ub}          & Maximum ramp up on operating cond.(Float, p.u./hr) & Y & S & $p^{ru}$ \\
  {\tt pg\_ramp\_lb}          & Maximum ramp down on operating cond.(Float, pu/hr) & Y & S & $p^{rd}$ \\
...
%   \hline    
%   {\tt\color{red} on\_status} & On status indicator for initial time step (Binary: 0/1) & Y & S & $u^{on,0}$\\
%   {\tt\color{red} select\_config}  & Active configuration uid for initial time step (uid) & Y & S & $u^{md,0}$\\
%   {\tt\color{red} pg} & Active dispatch for initial time step (Float, p.u.) & Y & S & $p^0$ \\
...
%   {\tt select\_ub} & Configuration selection upper bound (Binary: 0/1) & Y & T & $u^{md,max}$\\
%   {\tt select\_lb} & Configuration selection lower bound (Binary: 0/1) & Y & T & $u^{md,min}$\\
%   {\tt\color{red} feasible\_transition} & Uids of feasible configurations to transit to (Array of uids) & Y & S & $u^{tr,min}, u^{tr,max}$\\
%   {\tt pg\_ub} & Upper bound of active dispatch (Float, p.u.) & Y & T & $p^{max}$ \\
%   {\tt pg\_lb} & Lower bound of active dispatch (Float, p.u.) & Y & T & $p^{min}$ \\
%   {\tt qg\_ub} & Upper bound of reactive dispatch (Float, p.u.)& Y & T & $q^{max}$\\
%   {\tt qg\_lb} & Lower bound of reactive dispatch (Float, p.u.)& Y & T & $q^{min}$\\ 
...
%     &  each cost block is an array with exactly two elements:   &  &   &  \\
%     &  a) generation cost (\$/p.u.-hr), and b) block size (p.u.) &  &   &$c^{en}, p^{max}$ \\
%   {\tt on\_cost} & Dispatchable device fixed operating cost (Float) & Y & S & \\
%                             & $\hookrightarrow$ Case {\tt config\_num} $= 1$: &  &  & $c^{on}$\\
%                             & $\hookrightarrow$ Case {\tt config\_num} $> 1$: &  &  & $c^{md}$\\
...
%  &  FLEXI-RAMP-UP, FLEXI-RAMP-DOWN ) &  & & \\
  {\tt\color{red} REG\_UP} & Regulation reserve up requirement (Fraction, Float) & Y & S & $\sigma^{rgu}$ \\
  {\tt\color{red} REG\_DOWN} & Regulation reserve down requirement (Fraction, Float) & Y & S & $\sigma^{rgd}$ \\
  {\tt\color{red} SPIN} & Spinning reserve requirement (Fraction, Float) & Y & S & $\sigma^{spr}$ \\
  {\tt\color{red} NON\_SPIN} & Non-spinning reserve requirement (Fraction, Float) & Y & S & $\sigma^{nsp}$ \\
  {\tt\color{red} FLEXI\_RAMP\_UP} & Flexible-ramp up requirement (Float) & Y & S & $p^{fru,min}$ \\
  {\tt\color{red} FLEXI\_RAMP\_DOWN} & Flexible-ramp down requirement (Float) & Y & S & $p^{frd,min}$ \\
  \hline
  {\tt\color{red} REG\_UP\_cost} & Regulation reserve up marginal cost (\$/pu-hr, Float) & Y & S & $c^{rgu}$ \\
  {\tt\color{red} REG\_DOWN\_cost} & Regulation reserve down marginal cost (\$/pu-hr, Float) & Y & S & $c^{rgd}$ \\
  {\tt\color{red} SPIN\_cost} & Spinning reserve marginal cost (\$/pu-hr, Float) & Y & S & $c^{spr}$ \\
  {\tt\color{red} NON\_SPIN\_cost} & Non-spinning reserve marginal cost (\$/pu-hr, Float) & Y & S & $c^{nsp}$ \\
  {\tt\color{red} FLEXI\_RAMP\_UP\_online\_cost} & Flexible-ramp up marginal cost when online  & Y & S & $c^{fru,on}$ \\
  &(\$/pu-hr, Float) & && \\
  {\tt\color{red} FLEXI\_RAMP\_DOWN\_online\_cost} & Flexible-ramp down marginal cost when online & Y & S & $c^{frd,on}$ \\
    &(\$/pu-hr, Float) & && \\
  {\tt\color{red} FLEXI\_RAMP\_UP\_offline\_cost} & Flexible-ramp up marginal cost when offline & Y & S & $c^{fru,off}$ \\
    &(\$/pu-hr, Float) & && \\
  {\tt\color{red} FLEXI\_RAMP\_DOWN\_offline\_cost} & Flexible-ramp down marginal cost when offline & Y & S & $c^{frd,off}$ \\
    &(\$/pu-hr, Float) & && \\
  \hline
\end{tabular}
\end{center}    
