...
Below shows a high-level schematic description for the network object in JSON:
\begin{verbatim}
"network”: {
...
\label{sec:generator}
% A dispatchable device
% produces active power and reactive power, and supports voltage (target voltage set-point).
% Its output is usually limited 
% according to a PQ-curve,
% which specifies minimum and maximum reactive power values for every active power value. 
% Figure \ref{fig:generator} illustrates a generator (a) and feasible PQ region (shaded gray area) of its PQ-curve. 
% For simplicity, many modeling formulations would also
% approximate the PQ curve (gray area) by
% representing only its maximum and minimum active and reactive power.
% \begin{figure}[h]
% 	\includegraphics[width=0.9\linewidth]{img/gen_new2}
% 	\caption{\label{fig:generator} Illustration of a generator (a) and its PQ-curve (b).}
% \end{figure}
% Attributes for generators are listed in Section~\ref{nom:gen}.
% Below shows a schematic example on the static parameters for generators.
