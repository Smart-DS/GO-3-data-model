...
\date{April 29, 2022 - \underline{Next Upcoming Release: v0.8.3}}
...
% \begin{enumerate}
%     \item Update initial status attributes
%     \item (Archive) Multi-mode simplification\\
%     (all attributes that are:
%     1) waiting for discussion or 2) need confirmations are marked blue)
%     \item (Archive) Storage components\\
%     data formulation pending
%     \item (Archive) Sub devices\\
...
\hline
  {\tt time\_periods} & The number of time periods (Array of Int) & Y & T & $\lvert T \rvert$ \\
  {\tt interval\_duration} & Time duration of the intervals, per time period in hours & Y & T & $d_t$ \\
                           & (Array of Float) &  &  & \\
                           \hline
  {\tt timestamp\_start} & Period beginning timestamp for the first interval (Timestamp) & N & S & \\
  {\tt timestamp\_stop} & Period beginning timestamp for the interval following the last interval (Timestamp) & N & S & \\
\hline
...
\hline
  ...
  {\tt solar\_availability} & How solar availablity compares to other times of the year/season (String: High, Average, Low) & N & S & \\
  {\tt wind\_availability} & How solar availablity compares to other times of the year/season (String: High, Average, Low) & N & S & \\
...
\hline
  ...
  {\tt startups\_ub} & Array of time interval startup data blocks, where each & Y & S & $u^{su,max}$\\
                     ...
                     & 3) maximum startups within the interval (Int)  &   &   & \\
  ...
  {\tt\color{red} p\_startup\_ramp\_ub} & {(Case: producer) Max production ramp up when start up (Float, p.u./hr)} & Y & S & $p^{ru,max,su}$\\
                             & {(Case: consumer) Max consumption ramp up when start up (Float, p.u./hr)} & Y & S & $p^{ru,max,su}$\\
  {\tt\color{red} p\_shutdown\_ramp\_ub}& {(Case: producer) Max production ramp down when shut down (Float, pu/hr)} & Y & S & $p^{rd,max,sd}$\\
...
\hline
  ...
  {\tt cost} &  Array of cost blocks (Array of Array of Float), where & Y & T &  \\
...
\small
\begin{tabular}{ l | l | c | c | c |}
Active zonal reserve attributes & Description (Type) & Req & Sec & Sym\\
\hline
